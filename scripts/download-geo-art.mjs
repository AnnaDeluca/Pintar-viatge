...
const targets = [
  { n: 'klee',     f: 'Burg und Sonne - Klee.jpg' },
  { n: 'delaunay', f: 'Robert Delaunay - Rythme, Joie de vivre.jpg' },
  { n: 'doesburg', f: 'Theo van Doesburg Counter-CompositionV (1924).jpg' },
]

for (const { n, f } of targets) {
  await delay(600)
  const u800 = await api(f, 800)
  await delay(400)
  const u400 = await api(f, 400)
